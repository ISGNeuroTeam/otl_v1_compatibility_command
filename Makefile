...
all:
 ...
 test - run all tests\n\
 pack - make output archive, file name format \"otl_v1_vX.Y.Z_BRANCHNAME.tar.gz\"\n\
"
...
pack: make_build
	rm -f otl_v1-*.tar.gz
	echo Create archive \"otl_v1-$(VERSION)-$(BRANCH).tar.gz\"
	cd make_build; tar czf ../otl_v1-$(VERSION)-$(BRANCH).tar.gz otl_v1
...
otl_v1.tar.gz: build
	cd make_build; tar czf ../otl_v1.tar.gz otl_v1 && rm -rf ../make_build
