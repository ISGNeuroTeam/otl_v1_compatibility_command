...
all:
 ...
 test - run all tests\n\
 pack - make output archive, file name format \"pp_cmd_otl_v1_vX.Y.Z_BRANCHNAME.tar.gz\"\n\
"
...
pack: make_build
	rm -f pp_cmd_otl_v1-*.tar.gz
	echo Create archive \"pp_cmd_otl_v1-$(VERSION)-$(BRANCH).tar.gz\"
	cd make_build; tar czf ../pp_cmd_otl_v1-$(VERSION)-$(BRANCH).tar.gz otl_v1

clean_pack:
	rm -f pp_cmd_otl_v1-*.tar.gz


pp_cmd_otl_v1.tar.gz: build
	cd make_build; tar czf ../pp_cmd_otl_v1.tar.gz otl_v1 && rm -rf ../make_build
...
make_build:
	...
	echo make_build
	mkdir make_build
	cp -R ./otl_v1 make_build
	cp *.md make_build/otl_v1/
...
dev: venv
	rm -rf ./venv/lib/python3.9/site-packages/postprocessing_sdk/pp_cmd/otl_v1
